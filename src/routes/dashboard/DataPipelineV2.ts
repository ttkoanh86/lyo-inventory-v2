...
export interface ProductV2 {
    ...
    has_mac?: boolean;          // Cờ đánh dấu đã từng nhập kho có giá vốn (MAC > 0)
...
// 🟢 KHÔI PHỤC CHÍNH XÁC HÀM PARSE NGÀY GỐC (CÓ .replace(" ", "T") ĐỂ KHÔNG BAO GIỜ LỖI TIMESTAMP)
...
export function calculate_restock_data(
    ...
    for (let record of records) {
        ...
        // 🟢 ĐÁNH DẤU SẢN PHẨM ĐÃ TỪNG CÓ LỊCH SỬ BÁN HÀNG
        if (clean_sku) {
            variant_by_id.forEach((v) => {
                if (v.sku && v.sku.trim() === clean_sku) {
                    v.has_order_history = true;
                }
            });
        }
...
// BẮT BỤC SẢN PHẨM PHẢI ĐÃ TỪNG CÓ GIÁ VỐN MAC > 0 HOẶC ĐÃ TỪNG BÁN MỚI ĐƯỢC VÀO TAB NÀY
            ...
            if (resp.status == 200) {
                ...
                products.forEach((product: any) => {
                    ...
                    product.variants.forEach((variant: any) => {
                        ...
                        // 🟢 GIÁ VỐN BÌNH QUÂN MAC > 0 LÀ BẰNG CHỨNG XÁC NHẬN SẢN PHẨM ĐÃ TỪNG ĐƯỢC NHẬP KHO THỰC TẾ
                        if (max_mac > 0) {
...
// 🟢 KHÔI PHỤC HOÀN TOÀN HÀM CÀO ĐƠN GỐC CHẠY MƯỢT MÀ VÀ TỰ ĐỘNG NGẮT THEO 31 NGÀY
            ...
            if (resp.status === 200) {
                ...
                for (const order of orders) {
                    if (order.status !== "cancelled") {
                        ...
                        // 🟢 DÙNG parseSapoDate ĐÃ CHUẨN HÓA .replace(" ", "T")
                        const order_ts = parseSapoDate(date_str);

                        // GẶP ĐƠN VƯỢT QUÁ 31 NGÀY -> NGẮT VÒNG LẶP DỪNG CÀO
